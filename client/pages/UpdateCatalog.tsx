...
export default function UpdateCatalog() {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      const [teamsRes, itemRes] = await Promise.all([
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 ease-out">
      <div className="flex items-center space-x-4">
        ...
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold tracking-tight">Edit Catalog Item</h1>
          <p className="text-zinc-500 text-sm">Update the details of this service.</p>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-xl shadow-sm p-6">
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="space-y-2">
            ...
            <Input id="name" value={name} onChange={(e) => setName(e.target.value)} required className="max-w-md" />
          </div>
